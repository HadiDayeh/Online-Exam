...
if (empty($_POST)) {
    header("Location: /instructor/pages/password.php");
    exit();
}
...
try {
    ...
    $sql = "SELECT * FROM instructors WHERE instructors.id_instructor = ?";
    ...
    if (password_verify($old_password, $hash_old_password)) {
        ...
        if ($new_password === $confirm_password) {
            ...
            $sql = "UPDATE instructors SET password = ? WHERE id_instructor = ?";
            $stmt = $con->prepare($sql);
            $stmt->execute([$hash_new_password, $_SESSION["instructor_id"]]);
